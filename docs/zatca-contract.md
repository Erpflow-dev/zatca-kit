...
XML) — that is the copy we must archive, not the one we sent. Note ZATCA's
own typo `erroMessages` (and `erros` in the compliance schema); mirror their
spelling in the DTO or the field silently reads as undefined.
